...
- ```js
  ...
  ```
	- I am not sure I understand what `t` is. Is it a mutex with revision history that you update through transactions ?
	  ...
	  1. Data that goes into CAR is held in memory anyhow so there is no real benefit to incrementally writable interface vs batched write interface. Which is why benefit of the above
-