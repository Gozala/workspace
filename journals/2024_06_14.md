...
- `VirtualDOM<Input>` is written into `view` cell that host can render into desired target HTML element.
	- ℹ️ `Input` type parameter describes events that view can produce which host will forward to an `inbox` cell. Event handlers are defined in terms of decoders that extract certain data from the event and tag it so it can be rec